...
- Demo lifecycle workflow name/jobs labelled **NON-PILOT** (auth off, TES noop, stubs). Green CI is not certification.
...
- **Pi-class MinION chain** — `deploy/scripts/ci-minion-field-chain-e2e.sh` / `make minion-chain`: edge gateway with a memory cap, simulated POD5 stubs into a watch folder, `ferrum ingest watch`, DRS `ont_metrics`, then HelixTest `--mode ferrum-africa --africa-profile ont`. Not USB, not MinKNOW, not a real Pi. Africa CI job `africa-ont` runs the chain.
...
- **Build** — `ferrum-discovery` vendors GA4GH Service Info types (Apache-2.0). Optional `ga4gh-clearinghouse` is a git pin (`ga4gh-infra-v0.2.2`) so default `cargo build` does not need a sibling checkout.
- **HelixTest pin** — `HELIXTEST_REF=v0.1.1`. **ga4gh-infra pin** — `GA4GH_INFRA_REF=ga4gh-infra-v0.2.2` (`e43bf08…`; Docker Release vendors `docker/vendor`).
