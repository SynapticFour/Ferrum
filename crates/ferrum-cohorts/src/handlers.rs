...
pub async fn list_samples(
    ...
    let samples: Vec<CohortSample> = rows
        ...
        .take(limit as usize)
        .map(
            |(cid, cohort_id, sample_id, drs_object_ids, phenotype, added_at, added_by)| {
                sample_from_row(
                    cid,
                    cohort_id,
                    sample_id,
...
pub async fn get_schema(
    State(state): State<Arc<AppState>>,
) -> Result<Json<Vec<PhenotypeSchemaField>>> {
    let rows = state.repo.list_phenotype_schema().await?;
    let schema: Vec<PhenotypeSchemaField> = rows
        .into_iter()
        .map(
            |(id, field_name, display_name, field_type, ontology, required, description)| {
                PhenotypeSchemaField {
                    id,
                    field_name,
                    display_name,
                    field_type,
...
pub async fn cohort_stats(
        ...
        if let Some(obj) = phenotype.as_object() {
            for (k, v) in obj {
                let filled = !v.is_null()
                    ...
                    && !v
                        .as_f64()
                        .map(|f| f.to_string() == "0" && v.as_i64() == Some(0))
                        .unwrap_or(false);
                if filled {
    ...
    let phenotype_completeness: std::collections::HashMap<String, f64> = field_filled
        .into_iter()
        .map(|(k, count)| (k, count as f64 / n))
        .collect();
    Ok(Json(CohortStats {
        cohort_id: id,
        sample_count,
...
pub async fn list_versions(
    ...
    let versions: Vec<CohortVersionInfo> = rows
        .into_iter()
        .map(
            |(vid, version, created_at, created_by, note)| CohortVersionInfo {
                id: vid,
                cohort_id: id.clone(),
                version,
                created_at,
