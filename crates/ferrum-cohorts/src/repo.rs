...
impl CohortRepo {
    ...
    pub async fn cohort_accessible_by(&self, cohort_id: &str, sub: &str) -> Result<bool> {
        let row: Option<(String, Option<String>)> =
            sqlx::query_as("SELECT owner_sub, workspace_id FROM cohorts WHERE id = $1")
                .bind(cohort_id)
                .fetch_optional(&self.pool)
                .await?;
        ...
        let member: Option<(i32,)> =
            sqlx::query_as("SELECT 1 FROM cohort_members WHERE cohort_id = $1 AND sub = $2")
                .bind(cohort_id)
                .bind(sub)
                .fetch_optional(&self.pool)
                .await?;
    ...
    pub async fn update_cohort(
        ...
        let row = self
            .get_cohort(id)
            .await?
            .ok_or_else(|| crate::error::CohortError::NotFound(id.to_string()))?;
        let (_, name_cur, desc_cur, _, _, _, frozen, _, tags_cur, criteria_cur, _, _) = row;
        if frozen {
            return Err(crate::error::CohortError::Validation(
                "cohort is frozen".into(),
            ));
        }
    ...
    pub async fn update_sample(
        ...
        let cur = self
            ...
            .await?
            .ok_or_else(|| {
                crate::error::CohortError::NotFound(format!("sample {} in cohort", sample_id))
            })?;
    ...
    async fn recount_samples(&self, cohort_id: &str) -> Result<()> {
        let (count,): (i64,) =
            sqlx::query_as("SELECT COUNT(*)::bigint FROM cohort_samples WHERE cohort_id = $1")
                .bind(cohort_id)
                .fetch_one(&self.pool)
                .await?;
    ...
    pub async fn list_versions(
        &self,
        cohort_id: &str,
    ) -> Result<Vec<(String, i32, DateTime<Utc>, String, Option<String>)>> {
        let rows = sqlx::query_as(
