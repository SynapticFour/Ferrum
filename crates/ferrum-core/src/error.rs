...
impl IntoResponse for FerrumError {
    fn into_response(self) -> Response {
        let (status, message) = match &self {
            ...
            FerrumError::DatabaseError(_)
            | FerrumError::MigrationError(_)
            | FerrumError::StorageError(_)
            | FerrumError::EncryptionError(_) => (
            ...
            FerrumError::Config(_) => (
                StatusCode::INTERNAL_SERVER_ERROR,
                "Configuration error".to_string(),
            ),
            FerrumError::Internal(_) => (
                StatusCode::INTERNAL_SERVER_ERROR,
                "Internal server error".to_string(),
            ),
            FerrumError::PathTraversal => (
