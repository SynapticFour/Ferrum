        ...
        for n in &self.nodes {
            let (id, label) = match n {
                ProvenanceNode::DrsObject { id, name, .. } => {
                    (node_id(&NodeType::DrsObject, id), name.as_deref().unwrap_or(id).replace('"', "'"))
                }
                ProvenanceNode::WesRun { id, workflow_type, workflow_url, .. } => {
    ...
    pub fn to_dot(&self) -> String {
        ...
        for n in &self.nodes {
            let (id, label, shape) = match n {
                ProvenanceNode::DrsObject { id, name, .. } => {
                    (node_id(&NodeType::DrsObject, id), name.as_deref().unwrap_or(id).replace('"', "\\\""), "box")
                }
                ProvenanceNode::WesRun { id, workflow_type, workflow_url, .. } => {
                    let lbl = workflow_type.as_deref().or(workflow_url.as_deref()).unwrap_or(id);
                    (node_id(&NodeType::WesRun, id), lbl.replace('"', "\\\""), "ellipse")
                }
            };
        ...
        for e in &self.edges {
            let from = node_id(&e.from_type, &e.from_id);
            let to = node_id(&e.to_type, &e.to_id);
            lines.push(format!("  {} -> {} [label=\"{}\"];", from, to, e.edge_type.as_str()));
        }
...
impl ProvenanceStore {
    ...
    pub async fn record_derived_from(&self, from_object_id: &str, to_object_id: &str) -> Result<()> {
        self.insert_edge(
            NodeType::DrsObject,
            from_object_id,
            NodeType::DrsObject,
