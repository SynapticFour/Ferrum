...
pub fn safe_join(base: &Path, user_segment: &str) -> Result<PathBuf> {
    ...
    let canonical_base = base.canonicalize().map_err(|_| FerrumError::PathTraversal)?;
    let canonical = joined.canonicalize().map_err(|_| FerrumError::PathTraversal)?;
...
pub fn validate_drs_name(name: &str) -> Result<()> {
    if name.is_empty() || name.len() > 255 {
        return Err(FerrumError::ValidationError("name length must be 1-255".to_string()));
    }
    if name.contains('\0') || name.chars().any(|c| c.is_control()) {
        return Err(FerrumError::ValidationError("name must not contain control characters".to_string()));
    }
    if !name.chars().all(|c| c.is_ascii_alphanumeric() || "._-/: ".contains(c)) {
        return Err(FerrumError::ValidationError("name has invalid characters".to_string()));
    }
    Ok(())
}
