...
pub fn validate_url_ssrf(url: &str, policy: &SsrfPolicy) -> Result<()> {
    let parsed = Url::parse(url).map_err(|_| FerrumError::ValidationError("invalid URL".to_string()))?;
    let scheme = parsed.scheme().to_lowercase();
    if !policy.allowed_schemes.iter().any(|s| s.to_lowercase() == scheme) {
    ...
    if let Ok(ip) = host.parse::<IpAddr>() {
        if !policy.allow_private_networks && is_private_ip(&ip) {
            return Err(FerrumError::SsrfBlocked("private network not allowed".to_string()));
        }
    }
    Ok(())
}
...
impl SafeHttpClient {
    ...
    pub async fn post_json<T: serde::Serialize + ?Sized>(&self, url: &str, body: &T) -> Result<reqwest::Response> {
        validate_url_ssrf(url, &self.policy)?;
        self.inner
            .post(url)
            .json(body)
