...
impl LocalStorage {
    ...
    fn path_for(&self, key: &str) -> Result<PathBuf> {
        ...
        if path.strip_prefix(&self.base_path).is_err() {
            return Err(FerrumError::ValidationError("invalid key: path escape".to_string()));
        }
        Ok(path)
    }
}
...
impl ObjectStorage for LocalStorage {
    ...
    async fn delete(&self, key: &str) -> Result<()> {
        let path = self.path_for(key)?;
        if path.exists() {
            tokio::fs::remove_file(&path).await.map_err(|e| FerrumError::StorageError(e.into()))?;
        }
        Ok(())
    }
...
impl S3Storage {
    ...
    pub fn from_config(cfg: &crate::config::StorageConfig) -> impl std::future::Future<Output = Result<Self>> + Send {
        let endpoint = cfg.s3_endpoint.clone();
        let region = cfg.s3_region.clone().unwrap_or_else(|| "us-east-1".to_string());
        let bucket = cfg.s3_bucket.clone().unwrap_or_else(|| "ferrum".to_string());
