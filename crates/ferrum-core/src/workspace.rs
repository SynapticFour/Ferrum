...
pub async fn is_workspace_editor_or_owner(pool: &sqlx::PgPool, workspace_id: &str, sub: &str) -> Result<bool> {
    let role = get_workspace_member_role(pool, workspace_id, sub).await?;
    Ok(role.as_deref().map(|r| r == "owner" || r == "editor").unwrap_or(false))
}
