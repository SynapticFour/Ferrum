...
use crypt4gh::keys::{get_private_key, get_public_key, generate_keys as c4gh_generate_keys};
...
impl LocalKeyStore {
    ...
    fn path_private(&self, key_id: &str) -> std::path::PathBuf {
        self.base_path.join(sanitize_key_id(key_id)).with_extension("sec")
    }

    fn path_public(&self, key_id: &str) -> std::path::PathBuf {
        self.base_path.join(sanitize_key_id(key_id)).with_extension("pub")
    }
}
...
impl KeyStore for LocalKeyStore {
    async fn get_private_key(&self, key_id: &str) -> Result<Option<Vec<C4ghKeys>>> {
        ...
        let keys = C4ghKeys { method: 0, privkey: raw, recipient_pubkey: vec![] };
        Ok(Some(vec![keys]))
    }
    ...
    async fn store_key(&self, _key_id: &str, _keys: &[C4ghKeys]) -> Result<()> {
        ...
        Err(Crypt4GHError::KeyError("LocalKeyStore is read-only".to_string()))
    }
}
...
impl<DB: Send + Sync> KeyStore for DatabaseKeyStore<DB> {
    ...
    async fn store_key(&self, _key_id: &str, _keys: &[C4ghKeys]) -> Result<()> {
        Err(Crypt4GHError::KeyError("DatabaseKeyStore not yet implemented".to_string()))
    }
}
    ...
    W: AsyncWrite + Unpin + Send + 'static,
{
    ...
    read_task.await.map_err(|e| Crypt4GHError::Other(e.into()))??;
    write_task.await.map_err(|e| Crypt4GHError::Other(e.into()))??;
    Ok(())
}
...
impl std::io::Write for ChannelWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        if self.buffer.len() >= STREAM_CHUNK {
            ...
            self.sender.send(chunk).map_err(|_| std::io::ErrorKind::BrokenPipe)?;
        }
        Ok(buf.len())
    }
