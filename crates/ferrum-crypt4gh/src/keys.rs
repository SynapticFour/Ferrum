...
async fn key_exchange_handler(
    ...
    let visas: Vec<VisaObject> = match &claims {
        AuthClaims::Jwt { .. } => return Err(Crypt4GHError::Forbidden("GA4GH Passport required".to_string())),
    ...
    if !state.policy_engine.check(&body.object_id, &visas, subject_id) {
        return Err(Crypt4GHError::Forbidden("No valid visa for this object".to_string()));
    }
    ...
    let object_bytes = state
        .object_fetcher
        .as_ref()
        .ok_or(Crypt4GHError::NotFound("Object fetcher not configured".to_string()))?
