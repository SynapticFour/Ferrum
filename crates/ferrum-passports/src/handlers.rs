...
use crate::keys::{
    default_access_token_header, default_passport_header, default_visa_header, SigningKeys,
};
...
pub async fn token(
    ...
    let no_cache = [(CACHE_CONTROL, "no-cache, no-store"), (PRAGMA, "no-cache")];
    if form.grant_type == "urn:ietf:params:oauth:grant-type:token-exchange"
        && form.requested_token_type.as_deref()
            == Some("urn:ga4gh:params:oauth:token-type:passport")
            ...
            == Some("urn:ietf:params:oauth:token-type:access_token")
    {
        ...
        let passport_jwt =
            build_passport_jwt(&state, &decoded.claims.sub, &decoded.claims.iss).await?;
        return Ok((
    ...
    if form.grant_type == "authorization_code" {
        let code = form
            .code
            .as_deref()
            .ok_or_else(|| PassportError::Validation("code required".into()))?;
...
async fn build_visa_jwts(state: &AppState, sub: &str, _iss: &str) -> Result<Vec<String>> {
    let grants = state
        .repo
        .list_visa_grants(sub, state.config.issuer_base_url.trim_end_matches('/'))
        .await?;
    let issuer = state
        .config
        .issuer_base_url
...
pub async fn jwks(State(state): State<Arc<AppState>>) -> Result<Json<serde_json::Value>> {
    ...
    let v: serde_json::Value =
        serde_json::from_str(&json).map_err(|e| PassportError::Jwt(e.to_string()))?;
    Ok(Json(v))
}
...
pub async fn admin_list_visa_grants(
    ...
    let rows = state
        .repo
        .list_all_visa_grants(user_sub, dataset_id)
        .await?;
