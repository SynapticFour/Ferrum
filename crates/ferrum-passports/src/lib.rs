...
use crate::handlers::{
    admin_create_visa_grant, admin_delete_visa_grant, admin_list_visa_grants, authorize,
    get_oidc_configuration, jwks, token, userinfo,
};
...
pub fn router(pool: sqlx::PgPool) -> Router {
    ...
    Router::new()
        ...
        .route(
            "/admin/visa_grants",
            get(admin_list_visa_grants).post(admin_create_visa_grant),
        )
        .route(
            "/admin/visa_grants/:id",
            axum::routing::delete(admin_delete_visa_grant),
        )
