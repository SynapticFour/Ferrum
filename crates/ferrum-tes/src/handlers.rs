...
pub async fn create_task(
    ...
    let inputs = body.inputs.as_ref().map(|v| serde_json::to_value(v).unwrap()).unwrap_or(serde_json::json!([]));
    let outputs = body.outputs.as_ref().map(|v| serde_json::to_value(v).unwrap()).unwrap_or(serde_json::json!([]));
    let executors = serde_json::to_value(&body.executors).unwrap();
    let resources = body.resources.as_ref();
    let volumes = body.volumes.as_ref().map(|v| serde_json::to_value(v).unwrap());
    ...
    app.repo
        ...
        .await?;
    app.repo.update_state(&id, crate::types::TaskState::Running).await?;
    let external_id = app.executor.run(&id, &body).await?;
    if let Some(ref ext) = external_id {
        app.repo.set_external_id(&id, ext, app.executor.name()).await?;
    }
...
pub async fn get_task(
    ...
    let row = app.repo.get(&id).await?.ok_or_else(|| TesError::NotFound(format!("task not found: {}", id)))?;
    let (task_id, state, _name, _description, _inputs, _outputs, _executors, _resources, _volumes, _tags, _started_at, _ended_at, _created_at, external_id, _backend, _logs) = row;
    ...
    let (_, state_str, name2, desc2, inputs2, outputs2, executors2, resources2, volumes2, tags2, _, _, _, _, _, logs2) = app.repo.get(&id).await?.unwrap();
...
pub async fn cancel_task(
    ...
    if state == "RUNNING" || state == "QUEUED" || state == "INITIALIZING" {
        ...
        app.repo.update_state(&id, crate::types::TaskState::Canceled).await?;
    }
    Ok(Json(CreateTaskResponse { id }))
}
