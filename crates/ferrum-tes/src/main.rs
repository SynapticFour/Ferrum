...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let pool = match db {
        ...
        ferrum_core::DatabasePool::Sqlite(_) => return Err(anyhow::anyhow!("TES requires PostgreSQL").into()),
    };
    let backend = std::env::var("TES_BACKEND").ok();
    let work_dir = std::env::var("TES_WORK_DIR").ok().map(std::path::PathBuf::from);
    let app = ferrum_tes::router(pool, backend, work_dir);

    let bind: SocketAddr = std::env::var("BIND").unwrap_or_else(|_| "0.0.0.0:8084".to_string()).parse()?;
