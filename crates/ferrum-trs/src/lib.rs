...
use crate::handlers::{
    get_descriptor, get_descriptor_query, get_service_info, get_tool, get_tool_versions,
    list_tools, register_tool, ListToolsQuery,
};
...
pub fn router(pool: sqlx::PgPool) -> Router {
    ...
    Router::new()
        ...
        // GA4GH TRS OpenAPI: /tools/{id}/versions/{version_id}/{type}/descriptor
        .route(
            "/tools/:id/versions/:version_id/:descriptor_type/descriptor",
            get(get_descriptor),
        )
        // Non-standard alias (some clients): .../descriptor/{type}
        .route(
            "/tools/:id/versions/:version_id/descriptor/:descriptor_type",
            get(get_descriptor),
        )
        .route(
            "/tools/:id/versions/:version_id/descriptor",
            get(get_descriptor_query),
        )
