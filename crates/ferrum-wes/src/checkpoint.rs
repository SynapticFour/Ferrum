...
impl CheckpointStore {
    ...
    pub async fn check_cache(&self, task_hash: &str) -> Result<Option<Vec<String>>> {
        let row: Option<(Value,)> = sqlx::query_as(
            "SELECT drs_object_ids FROM wes_cache_entries WHERE task_hash = $1",
        )
        .bind(task_hash)
    ...
    pub async fn save_checkpoint(
        ...
        let base = self
            .drs_ingest_base_url
            .as_deref()
            .ok_or_else(|| crate::error::WesError::Validation("checkpoint storage not configured (no DRS ingest URL)".into()))?;
        ...
        for path in output_paths {
            ...
            let res = req.send().await.map_err(|e| crate::error::WesError::Other(e.into()))?;
            ...
            let json: serde_json::Value = res.json().await.map_err(|e| crate::error::WesError::Other(e.into()))?;
            let id = json
                .get("object_id")
                .and_then(|v| v.as_str())
                .map(String::from)
                .ok_or_else(|| crate::error::WesError::Other(anyhow::anyhow!("DRS ingest response missing object_id")))?;
            drs_ids.push(id);
        }
