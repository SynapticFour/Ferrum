...
impl WorkflowExecutor for CromwellExecutor {
    fn supported_languages(&self) -> Vec<(String, Vec<String>)> {
        vec![(
            "WDL".to_string(),
            vec!["1.0".to_string(), "1.1".to_string(), "draft-2".to_string()],
        )]
    }
    ...
    async fn submit(
        ...
        let _ = std::fs::write(
            &params_path,
            serde_json::to_string_pretty(&run.workflow_params).unwrap_or_else(|_| "{}".to_string()),
        );
        ...
        cmd.args([
            ...
            &workflow_url,
            "--inputs",
            params_path.to_str().unwrap_or("inputs.json"),
        ])
