...
impl MultiQCRunner {
    ...
    pub async fn run_for_completed_run(&self, run_id: &str) -> Result<Option<String>> {
        ...
        if let Some(obj) = outputs.get("ferrum:multiqc_status") {
            if obj.as_str().map(|s| s == "complete" || s == "skipped").unwrap_or(false) {
                return Ok(outputs
        ...
        tokio::fs::create_dir_all(&out_dir).await.map_err(WesError::Io)?;
        let run_multiqc = self.run_multiqc_container(&work_dir, &out_dir, run_id).await;
        ...
        let data_id = if data_path.is_file() {
            ...
            match self
                .ingest_file_multipart(
                    &ingest_url,
                    &format!("MultiQC Data — {} {}", workflow_type, &run_id[..run_id.len().min(8)]),
                    "report_data.zip",
                    "application/zip",
                    &data_bytes,
                    true,
    ...
    async fn run_multiqc_container(
        ...
        let args: Vec<String> = match runner.as_deref() {
            ...
            None => return Err(WesError::Other(anyhow::anyhow!("neither podman nor docker found for MultiQC"))),
        };
    ...
    async fn ingest_file_multipart(
        ...
        if !status.is_success() {
            ...
            return Err(WesError::Other(anyhow::anyhow!("DRS ingest failed: {} {}", status, body)));
        }
        let json: serde_json::Value = res.json().await.map_err(|e| WesError::Other(anyhow::anyhow!("{}", e)))?;
        let id = json
            .get("id")
            .and_then(|v| v.as_str())
