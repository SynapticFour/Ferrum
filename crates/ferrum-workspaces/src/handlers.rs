...
pub async fn create_workspace(
    ...
    let workspace = repo
        .create(&id, name, req.description.as_deref(), sub, &slug, &settings)
        .await?;
...
pub async fn get_workspace(
    ...
    let workspace = repo
        .get_by_id(&id)
        .await?
        .ok_or_else(|| WorkspaceError::NotFound(id))?;
    Ok(Json(workspace))
}
...
pub async fn add_member(
    ...
    let repo = WorkspaceRepo::new(state.pool.clone());
    repo.add_member(&id, &req.sub, role_parsed.as_str(), sub)
        .await?;
    let members = repo.list_members(&id).await?;
    let member = members
        .into_iter()
        .find(|m| m.sub == req.sub)
        .ok_or_else(|| WorkspaceError::Internal(anyhow::anyhow!("member not found")))?;
    state
        .activity
        .log(
            &id,
            ...
            Some(&req.sub),
            serde_json::json!({ "role": role_parsed.as_str() }),
        )
        .await?;
    Ok(Json(member))
}
...
pub async fn update_member_role(
    ...
    let role_parsed = WorkspaceRole::from_str(req.role.trim()).unwrap_or(WorkspaceRole::Viewer);
    let repo = WorkspaceRepo::new(state.pool.clone());
    repo.update_member_role(&id, &member_sub, role_parsed.as_str())
        .await?;
    let members = repo.list_members(&id).await?;
    let member = members
        .into_iter()
        .find(|m| m.sub == member_sub)
        .ok_or_else(|| WorkspaceError::NotFound("member".to_string()))?;
    Ok(Json(member))
}
...
pub async fn get_workspace_contents(
    ...
    role.require_read()?;
    let repo = WorkspaceRepo::new(state.pool.clone());
    let (total_size_bytes, drs_count, wes_count, cohort_count, active_runs) =
        repo.get_contents(&id).await?;
