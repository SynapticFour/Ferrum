...
impl WorkspaceRepo {
    ...
    pub async fn archive(&self, id: &str) -> Result<bool> {
        let r = sqlx::query(
            "UPDATE workspaces SET is_archived = true, updated_at = now() WHERE id = $1",
        )
        .bind(id)
        .execute(&self.pool)
        .await?;
    ...
    pub async fn update_member_role(
        ...
        let r = sqlx::query(
            "UPDATE workspace_members SET role = $3 WHERE workspace_id = $1 AND sub = $2",
        )
        .bind(workspace_id)
        .bind(sub)
        .bind(role)
    ...
    pub async fn get_activity(
        ...
        limit: i64,
        offset: i64,
    ) -> Result<Vec<WorkspaceActivityItem>> {
    ...
    pub async fn get_contents(
        ...
        .await?;
        let drs_count: (i64,) =
            sqlx::query_as("SELECT count(*) FROM drs_objects WHERE workspace_id = $1")
                .bind(workspace_id)
                .fetch_one(&self.pool)
                .await?;
        let wes_count: (i64,) =
            sqlx::query_as("SELECT count(*) FROM wes_runs WHERE workspace_id = $1")
                .bind(workspace_id)
                .fetch_one(&self.pool)
                .await?;
        ...
        .fetch_one(&self.pool)
        .await?;
        let cohort_count: (i64,) =
            sqlx::query_as("SELECT count(*) FROM cohorts WHERE workspace_id = $1")
                .bind(workspace_id)
                .fetch_one(&self.pool)
                .await?;
        Ok((
    ...
    pub async fn slug_exists(&self, slug: &str) -> Result<bool> {
        let row: Option<(bool,)> =
            sqlx::query_as("SELECT true FROM workspaces WHERE slug = $1 LIMIT 1")
                .bind(slug)
                .fetch_optional(&self.pool)
                .await?;
        Ok(row.is_some())
