#!/usr/bin/env bash
# End-to-end smoke test: `ferrum demo start --edge` (SQLite + local DRS ingest/stream).
...
mkdir -p "$TMP/objects"
printf '%s' 'GA4GH edge demo offline E2E payload' >"$PAYLOAD"
...
echo "ci-edge-demo-e2e: starting ferrum demo start --edge on ${BASE} (log: ${LOG})"
"$FERRUM_CLI" demo start --edge >>"$LOG" 2>&1 &
CLI_PID=$!
...
for i in $(seq 1 40); do
  ...
  if ! kill -0 "$CLI_PID" 2>/dev/null; then
    echo "ci-edge-demo-e2e: ferrum demo exited before health check (see log):" >&2
    tail -80 "$LOG" >&2 || true
    exit 1
  fi
  sleep 0.25
...
if [ "$ready" -ne 1 ]; then
  echo "ci-edge-demo-e2e: gateway not healthy after 10s" >&2
  tail -80 "$LOG" >&2 || true
  exit 1
fi

echo "ci-edge-demo-e2e: health OK"

code_si=$(curl -s -o /dev/null -w "%{http_code}" "${BASE}/ga4gh/drs/v1/service-info" || echo "000")
if [ "$code_si" != "200" ]; then
  echo "ci-edge-demo-e2e: DRS service-info expected 200, got ${code_si}" >&2
  exit 1
fi
