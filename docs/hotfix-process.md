...
```bash
LAST_TAG=v0.3.0   # letzter stabiler Release
git fetch --tags
git checkout -b hotfix/0.3.1 "${LAST_TAG}"
```
...
git checkout main
git pull
git merge --no-ff hotfix/0.3.1 -m "Merge hotfix/0.3.1"
```
...
```bash
# CHANGELOG.md: Eintrag unter [0.3.1]
git tag -a v0.3.1 -m "v0.3.1 — hotfix: <Kurzbeschreibung>"
...
**Betreff:** Ferrum Hotfix v0.3.1 — empfohlenes Update

```
Guten Tag,

wir haben Ferrum v0.3.1 veröffentlicht (Hotfix für: <Problem in einem Satz>).

Empfohlene Schritte:
1. Release und SHA256SUMS.txt von GitHub laden und verifizieren
2. In .env setzen: FERRUM_VERSION=v0.3.1  (Pflicht — install.sh startet sonst nicht)
3. Images importieren/starten: ./import.sh  (offline) oder ./install.sh
4. Prüfen: curl http://<host>:8080/health

Rollback: FERRUM_VERSION auf v0.3.0 setzen, erneut ./install.sh
