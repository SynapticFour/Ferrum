#!/usr/bin/env bash
# Build an optimized single-binary Ferrum gateway for Laptop Mode on this machine.
#
# Auto-detects OS/CPU and applies native optimizations when safe.
#
# Usage:
#   ./scripts/build-laptop-native.sh              # build only
#   ./scripts/build-laptop-native.sh --install    # install to ~/.ferrum/bin
#   ./scripts/build-laptop-native.sh --no-native-cpu   # generic CPU (portable on same arch)
#   ./scripts/build-laptop-native.sh --target aarch64-unknown-linux-gnu  # cross hint (no native CPU)
#
set -euo pipefail
...
echo "[ferrum] Laptop Mode native build"
echo "[ferrum]   OS:      $OS"
...
echo "[ferrum] Building ferrum-gateway (laptop feature set)..."
TARGET_DIR="$(cargo metadata --format-version 1 --no-deps | python3 -c 'import json,sys; print(json.load(sys.stdin)["target_directory"])')"
if [ "$USE_TARGET_FLAG" -eq 1 ]; then
  cargo build --profile "$PROFILE" -p ferrum-gateway \
    --target "$TARGET" \
    --no-default-features --features laptop
...
if [ "$INSTALL" -eq 1 ]; then
  ...
  echo "[ferrum] Run: ferrum demo start --offline"
fi
