...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const showBrokerMisconfig = Boolean(authConfig?.require_auth && !brokerLoginUrl);
  ...
  return (
    <div className={cn('min-h-screen bg-background flex flex-col', dark && 'dark')} dir={dir}>
      ...
      <main className={cn('flex-1 flex flex-col', dir === 'rtl' ? 'pr-56' : 'pl-56')}>
        {showBrokerMisconfig && (
          <div className="border-b border-destructive/40 bg-destructive/10 px-6 py-2 text-sm">
